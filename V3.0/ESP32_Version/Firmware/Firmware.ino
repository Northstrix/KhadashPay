...
byte pin_column[COLUMN_NUM] = {16, 17, 22, 25};
...
// Keys (Below)
String kderalgs = "P31JIH330o0viWT5Ns11S7Y22";
int numofkincr = 971;
byte hmackey[] = {"R4UWhs346aC7Z5LjJ6Fw10B37QnEXzToM9GF41r43tq1zl8AQyq8IL1t4pI0oyr5c1Q5o0jH492HbWJ3ai111oE0W2l05pICx1N3Gs96DVzhRhiw5lslOVD3MU29YJu8ze4QN2MguaIV5pFUjv"};
byte des_key[] = {
0xae,0xc4,0x72,0xba,0xf4,0x20,0xdb,0xf8,
0xae,0xf8,0xd0,0x2f,0xea,0xd7,0xbc,0xa1,
0x07,0x73,0x3b,0x2e,0xe5,0x7e,0xca,0x0e
};
uint8_t AES_key[32] = {
0x8e,0x3a,0xcd,0x00,
0x23,0x78,0xc0,0x16,
...
void display_letters_with_shifting_background() {
  ...
  if (chosen_lock_screen == 5){
    for (int i = 0; i < 312; i++){
      for (int j = 0; j < 61; j++){
        if (khadash_pay_icon[i][j] == 1)
          mvng_bc.drawPixel(i, j, Lion_Monument[(i + 4 + k)%320][j+120]);
      }
    }
  }
  ...
  if (chosen_lock_screen == 9){
    for (int i = 0; i < 312; i++){
      for (int j = 0; j < 61; j++){
        if (khadash_pay_icon[i][j] == 1)
          mvng_bc.drawPixel(i, j, Reykjavik[(i + 4 + k)%320][j+120]);
      }
    }
  }
  ...
  if (chosen_lock_screen == 11){
    for (int i = 0; i < 312; i++){
      for (int j = 0; j < 61; j++){
        if (khadash_pay_icon[i][j] == 1)
          mvng_bc.drawPixel(i, j, Vienna[(i + 4 + k)%320][j+120]);
      }
    }
  }
...
void display_lock_screen() {
  ...
  if (chosen_lock_screen == 5){
    for (int i = 0; i < 320; i++){
      for (int j = 0; j < 240; j++){
        tft.drawPixel(i, j, Lion_Monument[i][j]);
      }
    }
  }
  ...
  if (chosen_lock_screen == 9){
    for (int i = 0; i < 320; i++){
      for (int j = 0; j < 240; j++){
        tft.drawPixel(i, j, Reykjavik[i][j]);
      }
    }
  }
...
void lock_screen_keypad() {
  ...
  tft.setTextSize(1);
  tft.setTextColor(0xf7de);
  disp_centered_text_b_w("github.com/Northstrix/KhadashPay", 224);
...
void loop() {
  for (int i = 0; i < 312; i++) {
    for (int j = 0; j < 61; j++) {
      if (khadash_pay_icon[i][j] == 1)
        mvng_bc.drawPixel(i, j, Greenwich[(i + 4 + k) % 320][j + 98]);
    }
  }
  mvng_bc.pushSprite(4, 10, TFT_TRANSPARENT);
  char key = keypad.getKey();
